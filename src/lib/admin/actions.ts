...
import { isAutoGeneratedCertificate, generateProfessionalCertificate } from '@/lib/certificates/generator';
...
// 코스(상품) 목록 가져오기 (증명서 발급용)
export async function getCoursesForCertificates(): Promise<{
  success: boolean;
  courses?: { id: string; title: string; category: string; certificates: string[] | null }[];
  error?: string;
}> {
  ...
  const { data: courses, error } = await supabase
    .from('courses')
    .select('id, title, category, certificates')
    .eq('is_active', true)
    .order('title');
...
export interface IssuedCertificate {
  issuedAt: string;      // 발급 처리 시간
  issueDate: string;     // 문서에 표시될 발급일
  issuedBy?: string;
}
...
export async function issueCertificate(
  enrollmentId: string,
  certificateId: string,
  issueDate?: string  // 발급일 (지정하지 않으면 오늘 날짜)
): Promise<{ success: boolean; error?: string }> {
  const isAdmin = await checkIsAdmin();
  if (!isAdmin) {
    return { success: false, error: '관리자 권한이 필요합니다.' };
  ...
  // 발급일 설정 (지정하지 않으면 오늘 날짜)
  const issueDateValue = issueDate || new Date().toISOString().split('T')[0];
...
export async function previewCertificateForAdmin(
  enrollmentId: string,
  certificateId: string,
  customIssueDate?: string  // 미리보기용 발급일 (선택사항)
): Promise<{
  success: boolean;
  pdfBase64?: string;
  error?: string;
  ...
  // enrollment 조회 (issued_certificates 포함)
  const { data: enrollment, error: enrollmentError } = await supabase
    .from('enrollments')
    .select(`
      *,
  ...
  try {
    ...
    // 발급일 결정: 커스텀 발급일 > issued_certificates의 issueDate > 수료일
    const issuedCerts: IssuedCertificates = enrollment.issued_certificates || {};
    const issuedCert = issuedCerts[certificateId];

    let completionDate: Date;
    if (customIssueDate) {
      completionDate = new Date(customIssueDate);
    } else if (issuedCert?.issueDate) {
      completionDate = new Date(issuedCert.issueDate);
    } else {
      completionDate = new Date(enrollment.completed_at || new Date());
    }
