...
export async function issueCertificate(
  ...
  const currentIssued: IssuedCertificates = enrollment?.issued_certificates || {};

  // 이미 발급된 경우
  if (currentIssued[certificateId]) {
    return { success: false, error: '이미 발급된 증명서입니다.' };
  }

  // 발급 처리
  const updatedIssued: IssuedCertificates = {
    ...currentIssued,
    [certificateId]: {
  ...
  const { error: updateError } = await supabase
    .from('enrollments')
    .update({ issued_certificates: updatedIssued })
    .eq('id', enrollmentId);

  if (updateError) {
    return { success: false, error: '증명서 발급에 실패했습니다.' };
  }
...
// 발급된 증명서 목록 조회
export async function getIssuedCertificates(enrollmentId: string): Promise<{
  success: boolean;
  ...
  return { success: true, data: data?.issued_certificates || {} };
}
...
export async function revokeCertificate(
  ...
  if (!currentIssued[certificateId]) {
    return { success: false, error: '발급된 증명서가 없습니다.' };
  }

  const { [certificateId]: removed, ...remainingIssued } = currentIssued;

  const { error: updateError } = await supabase
    .from('enrollments')
    .update({ issued_certificates: remainingIssued })
    .eq('id', enrollmentId);

  if (updateError) {
    return { success: false, error: '증명서 발급 취소에 실패했습니다.' };
  }
...
// 관리자용 증명서 미리보기 생성
export async function previewCertificateForAdmin(
  enrollmentId: string,
  certificateId: string
): Promise<{
  success: boolean;
  ...
  console.log('previewCertificateForAdmin - enrollmentId:', enrollmentId, 'certificateId:', certificateId);
  ...
  console.log('previewCertificateForAdmin - enrollment:', enrollment, 'error:', enrollmentError);

  if (enrollmentError || !enrollment) {
    console.error('previewCertificateForAdmin - enrollment fetch error:', enrollmentError);
    return { success: false, error: '수강 정보를 찾을 수 없습니다.' };
  }

  // 사용자 정보 조회 (profiles 테이블)
  ...
  if (enrollment.status !== 'completed') {
    return { success: false, error: '수료 완료된 수강생만 증명서를 미리볼 수 있습니다.' };
  }

  try {
    const { generateProfessionalCertificate, isAutoGeneratedCertificate, generateVerificationCode, getCertificateSubtitle } = await import('@/lib/certificates/generator');
    const { getCertificateById } = await import('@/data/certificateTemplates');
    ...
    // 사용자 이름
    const userName = userProfile?.name || '수강자';

    // 수료일
    const completionDate = new Date(enrollment.completed_at || new Date());

    // 자동 생성 증명서인 경우 전문적인 디자인으로 생성
    ...
    // 템플릿 기반 증명서는 PDF 파일 로드하여 이름/날짜 삽입
